...
import { getConsistentFormatterColors, dimText, colorizeLevelText, boldText } from '../utils/formatterColors';
...
export class PrettyJsonFormatter implements LogFormatter {
  format(
    ...
    if (colors) {
      ...
      json = json
        .replace(/("timestamp":\s*")([^"]+)(")/, (_m, p1, p2, p3) => `${p1}${dimText(p2, colors)}${p3}`)
        .replace(/("level":\s*")([^"]+)(")/, (_m, p1, p2, p3) => `${p1}${colorizeLevelText(p2, entry.level, colors)}${p3}`)
        .replace(/("message":\s*")([^"]+)(")/, (_m, p1, p2, p3) => `${p1}${boldText(p2, colors)}${p3}`);
    }
    return json;
  }
}
