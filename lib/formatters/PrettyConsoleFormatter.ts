...
import { getConsistentFormatterColors, colorizeLevelText, dimText, type FormatterColors } from '../utils/formatterColors';
...
export class PrettyConsoleFormatter implements LogFormatter {
  ...
  format(entry: LogEntry, options?: { consoleColors?: CustomConsoleColors; useColors?: boolean }): string {
    ...
    // Message section
    if (entry.message) {
      const messageLabel = useColors 
        ? dimText('Message:', colors!)
        : 'Message:';
    ...
    if (entry.args.processedArgs.length > 0) {
      const argsLabel = useColors 
        ? dimText('Arguments:', colors!)
        : 'Arguments:';
      lines.push(`${argsLabel}`);
      
      entry.args.processedArgs.forEach((arg, index) => {
        const argType = this.getValueType(arg);
        const typeIndicator = useColors 
          ? dimText(`[${argType}]`, colors!)
          : `[${argType}]`;
   ...
   */
  private formatObject(obj: unknown, indentLevel: number, useColors: boolean, colors: FormatterColors | null): string {
    ...
    if (Array.isArray(obj)) {
      ...
      entries.forEach(([key, value]) => {
        ...
        const typeIndicator = useColors 
          ? dimText(`[${valueType}]`, colors!)
          : `[${valueType}]`;
        
        if (typeof value === 'object' && value !== null) {
          const keyLabel = useColors 
            ? this.colorizeKey(key, colors!)
            : key;
          ...
          if (stringValue.length > this.maxLineLength - childIndent.length - key.length - 10) {
            lines.push(`${childIndent}${keyLabel}: ${typeIndicator}`);
            const wrappedValue = this.wrapText(stringValue, this.maxLineLength - (indentLevel + 2) * this.indentSize);
            wrappedValue.forEach(line => {
  ...
  private getValueType(value: unknown): string {
    ...
    if (typeof value === 'object' && value !== null) {
      ...
      if (obj.name && obj.message && obj.stack && typeof obj.name === 'string' && typeof obj.message === 'string') {
        return 'error';
      }
