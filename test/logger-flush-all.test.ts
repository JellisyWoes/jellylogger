...
describe('Logger flushAll', () => {
  ...
  it('should handle flush errors gracefully', async () => {
    ...
    const consoleSpy = spyOn(consoleTransport, 'flush').mockRejectedValue(
      new Error('Console flush failed'),
    );
    const fileSpy = spyOn(fileTransport, 'flush').mockResolvedValue(undefined);
  ...
  it('should work with child logger flush', async () => {
    ...
    const _childLogger = logger.child({ messagePrefix: '[CHILD]' });
