...
describe('PrettyConsoleFormatter', () => {
  ...
  test('formats log entry with structured data', () => {
    const entry = createLogEntry(LogLevel.INFO, 'User action', [], {
      userId: 12345,
      action: 'login',
      metadata: {
        ip: '192.168.1.1',
  ...
  test('handles different data types correctly', () => {
    const entry = createLogEntry(LogLevel.DEBUG, 'Type test', [
      null,
      undefined,
      42,
      true,
  ...
  test('wraps long messages correctly', () => {
    const longMessage =
      'This is a very long message that should be wrapped across multiple lines when it exceeds the maximum line length configuration of the formatter.';
    ...
    const messageLines = lines.filter(
      line =>
        line.trim() && !line.includes('─') && !line.includes('Message:') && !line.includes('['),
    );
  ...
  test('handles nested objects with proper indentation', () => {
    const entry = createLogEntry(LogLevel.INFO, 'Nested data test', [], {
      level1: {
        level2: {
          level3: {
            deepValue: 'found it!',
  ...
  test('handles arrays properly', () => {
    const entry = createLogEntry(LogLevel.INFO, 'Array test', [], {
      items: ['first item', { id: 1, name: 'Object item' }, [1, 2, 3], null],
    });
  ...
  test('respects custom configuration options', () => {
    ...
    const entry = createLogEntry(LogLevel.INFO, 'Configuration test', [], { key: 'value' });
    ...
    if (dataLines.length > 0) {
      expect(dataLines[0]).toMatch(/^ {4}/); // 4 spaces for first level
    }
